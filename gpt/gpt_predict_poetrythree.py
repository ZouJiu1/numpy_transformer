...
from gpt.gpt_train_potry3000 import getdata, create_masks_future, getinputs
from net.layernorm import layer_norm
from net.fullconnect import fclayer
from gpt.classify_gpt import classify_layer
from net.flatten import flatten_layer

def predict(float16_):
    global savename
    ...
    la = int(savename[-5])
    layers += att_layer[:la] #, at5#, at6, at7, at8, at9, at10, at11]
    ...
    num_sample = 30
    example = []
    start_lens = [12, 16, 30, 20, 40]
    # inputcharacter = '0'
    for _ in range(num_sample):
        start_length = np.random.choice(start_lens)
        inputs, input_mask, label_single, id_start = getinputs(inputid, start_length, 1, input_texts, char2id, id2char)
        # inputcharacter = "".join([id2char[ci]  for ci in inputs[0]])
    ...
    character = list(r''',.? !;、，。？！；''')
    file_path = os.path.join(abspath, 'gpt', 'model', f'example_english{la}.txt')
    file = open(file_path, "w", encoding = 'utf-8')
    fileshow = ""
    file.write(f"{la} transformer layers\n\n")
    for i, sample in enumerate(example):
        ...
        for j in range(len(sample)):
            if j <= start_length - 2:
                ...
                fileshow += sample[i] + '\n Model Output: '
                continue
            if ln < 100 and ln > 50 and sample[j] in character:
                file.write(sample[j])
                fileshow += sample[j]
                file.write('\n ')
                fileshow += "\n "
...
if __name__ =="__main__":
    for i in range(1, 6):
        savename = f"gpt_poetry3000_last{i}.pkl"
        pretrained_model = os.path.join(abspath, r'gpt', r'model', savename)
